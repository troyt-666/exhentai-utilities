...
export class GeminiClient {
    ...
    async classifyArchive(archive) {
        ...
        try {
            const response = await axios.post(`https://generativelanguage.googleapis.com/v1beta/models/${this.config.model}:generateContent?key=${this.apiKey}`, {
                contents: [{
                        parts: [{
                                text: prompt
                            }]
...
- [アンソロジー] LQ -Little Queen- Vol.63 → {"category": "杂志", "subdirectory": "LQ -Little Queen-", "confidence": 0.9, "reasoning": "Anthology magazine"}`;
    ...
    async batchClassify(archives, batchSize = 5) {
        const results = [];
        for (let i = 0; i < archives.length; i += batchSize) {
            const batch = archives.slice(i, i + batchSize);
            const batchPromises = batch.map(archive => this.classifyArchive(archive).catch(error => {
